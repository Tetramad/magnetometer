...
int LIS2MDL_Measure(void) {
	...
	LOG_DBG("LIS2MDL: STATUS_REG: %u", buffer[1]);
	...
	const float mag_meas = sqrtf(
			powf(mag_x_meas, 2.0f) + powf(mag_y_meas, 2.0f)
					+ powf(mag_z_meas, 2.0f));

	LOG_DBG("LIS2MDL: TEMP_OUT_L_REG=%u", buffer[7]);
	LOG_DBG("LIS2MDL: TEMP_OUT_H_REG=%u", buffer[8]);
	...
	LOG_DBG("LIS2MDL: Magnetic Field (uT): %d", magneticFieldMagnitude_mT);
	LOG_DBG("LIS2MDL: Magnetic Field X(uT): %d", magneticFieldXAxis_mT);
	LOG_DBG("LIS2MDL: Magnetic Field Y(uT): %d", magneticFieldYAxis_mT);
	LOG_DBG("LIS2MDL: Magnetic Field Z(uT): %d", magneticFieldZAxis_mT);
	LOG_DBG("LIS2MDL: Temperature(m'C): %d", temperature_Celsius);
...
static int configure(void) {
	...
	status = HAL_I2C_Master_Transmit(&hi2c1, SLAVE_ADDRESS, buffer,
			sizeof(buffer), 25U);
	if (status != HAL_OK) {
		return -__LINE__;
	}
...
static int is_data_available(void) {
	...
	I2C_MEMADD_SIZE_8BIT, &status_reg, 1U, 25U);
	if (status != HAL_OK) {
		return -__LINE__;
	}
